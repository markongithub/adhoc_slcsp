...
rateAreasByZip zips = let
  makePair zipInst = (zipcode zipInst, (zipState zipInst, zipRateArea zipInst))
...
secondLowestCostByZIP zipTable rateTable zipStr
  | Map.notMember zipStr zipTable = Nothing
  | otherwise = secondLowestCost rateTable rateArea
  where rateArea = zipTable!zipStr

formattedSLCSP :: ZIPTable -> RateTable -> String -> String
formattedSLCSP zipTable rateTable zipCode =
  case (secondLowestCostByZIP zipTable rateTable zipCode) of
    Nothing -> ""
    Just amount -> printf "%.2f" amount
